...
const contextNotify contextKey = "notify"
...
func Test_ImmediateTaskExecution(t *testing.T) {
	...
	ctx, cancel := context.WithTimeout(context.WithValue(context.Background(), contextNotify, notify), 2*time.Second)
	defer cancel()
...
func Test_DelayedTaskExecution(t *testing.T) {
	...
	notify := make(chan int64, 1)
	ctx, cancel := context.WithTimeout(context.WithValue(context.Background(), contextNotify, notify), 3*time.Second)
	defer cancel()
